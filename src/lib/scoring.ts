 ...
 *
 *   benchmarkStandard — the performance worth 100 points
 *   benchmarkZero — the performance worth 0 points
 *
 * Direction falls out of the math. For a sprint, benchmarkStandard (say 5.0s) is
 * *below* benchmarkZero (9.0s), so the slope is negative and faster scores
 * higher — no "lower is better" flag needed anywhere.
 *
 * Points are floored at 0 but deliberately NOT capped at 100: beating the
...
export function isLowerBetter(config: ScoringConfig): boolean {
  return config.benchmarkStandard < config.benchmarkZero;
}

/**
 * An event whose two benchmarks are equal has no scale — every performance
 * would be both 0 and 100 points. Callers should reject these on save.
 */
export function isScorable(config: ScoringConfig): boolean {
  return (
    Number.isFinite(config.benchmarkStandard) &&
    Number.isFinite(config.benchmarkZero) &&
    config.benchmarkStandard !== config.benchmarkZero
  );
}
...
export function scoreResult(raw: number, config: ScoringConfig): number {
  ...
  const span = config.benchmarkStandard - config.benchmarkZero;
  const points = (100 * (raw - config.benchmarkZero)) / span;
...
export function rawForPoints(points: number, config: ScoringConfig): number | null {
  ...
  const span = config.benchmarkStandard - config.benchmarkZero;
  return config.benchmarkZero + (points * span) / 100;
}
...
export function describeScale(
  ...
  return `100 pts = ${formatMeasurement(config.benchmarkStandard, decimals)}${unit} · 0 pts = ${formatMeasurement(config.benchmarkZero, decimals)}${unit}`;
}
